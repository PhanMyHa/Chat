...
export const protectedRoute = (req, res, next)=>{
    try {
        ...
        jwt.verify(token, process.env.ACCESS_TOKEN_SECRET,async(err, decodeedUser)=>{
            if (err){
                console.error(err);
                return res.status(403).json({message:"accesstoken het han hoac khong dung"});
            }
            const user= await User.findById(decodeedUser.userId).select('-hashedPassword')
            //tim user
            if(!user){
